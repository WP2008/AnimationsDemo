...
@property (nonatomic) CAShapeLayer *arcToCircleLayer2;
...
- (void)doStep1 {
    ...
    // 用代理监听 动画的结束  不能使用属性比较的方法  因为delegate 中的对象时深拷贝对象   使用这种keyValue的方法存储
...
// 第1阶段 方法2   使用 CAShapeLayer 的 strock 方案
- (void)doStep1_2 {
    self.arcToCircleLayer2 = [CAShapeLayer layer];
    ...
    CGPoint point = CGPointMake(CGRectGetMidX(self.arcToCircleLayer2.bounds), CGRectGetMidY(self.arcToCircleLayer2.bounds));
    [path addArcWithCenter:point radius:kRadius startAngle:0 endAngle:M_PI * 2 clockwise:NO];
    self.arcToCircleLayer2.path = path.CGPath;
    self.arcToCircleLayer2.lineWidth = kLineWidth;
    self.arcToCircleLayer2.strokeColor = self.likeBlackColor.CGColor;
    self.arcToCircleLayer2.fillColor = nil;
    ...
    // transform status
    self.arcToCircleLayer2.transform = CATransform3DIdentity;
...
#pragma mark - animation step stop
 // 用代理监听 动画的结束  不能使用属性比较的方法  因为delegate 中的对象为深拷贝对象   所以使用这种keyValue的方法存储
